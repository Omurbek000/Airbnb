...
class Country(models.Model):
    country_name = models.CharField(
        max_length=30, unique=True, verbose_name="Название страны"
    )
    country_capital = models.CharField(
        max_length=30, unique=True, verbose_name="Столица"
    )
    country_currency = models.CharField(
        max_length=30, unique=True, verbose_name="Валюта"
    )
    country_image = models.ImageField(
        upload_to="country/images", blank=True, null=True, verbose_name="Флаг/Фото"
    )
...
class UserProfile(AbstractUser):
    ...
    user_role = models.CharField(
        max_length=10, choices=ROLE_CHOICES, default="Guest", verbose_name="Роль"
    )
    ...
    user_phone = PhoneNumberField(
        region="KG", null=True, blank=True, verbose_name="Телефон"
    )
...
class City(models.Model):
    ...
    city_country = models.ForeignKey(
        Country, on_delete=models.CASCADE, related_name="cities", verbose_name="Страна"
    )
...
class Amenity(models.Model):
    name = models.CharField(
        max_length=100, unique=True, verbose_name="Название удобства"
    )
    icon = models.ImageField(
        upload_to="amenities/", blank=True, null=True, verbose_name="Иконка"
    )
...
class Hotel(models.Model):
    ...
    hotel_city = models.ForeignKey(
        "City", on_delete=models.CASCADE, related_name="hotels", verbose_name="Город"
    )
    ...
    hotel_video = models.FileField(
        upload_to="hotel/video", null=True, blank=True, verbose_name="Видео"
    )
    hotel_price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Цена за ночь"
    )
    amenities = models.ManyToManyField(
        Amenity, related_name="hotels", blank=True, verbose_name="Удобства"
    )  # ✅ связь с удобствами
...
class HotelImage(models.Model):
    ...
    hotel_image = models.ImageField(
        upload_to="hotel_images/", null=True, blank=True, verbose_name="Фото"
    )
    uploaded_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата загрузки")

    def __str__(self):
        return "{self.hotel.hotel_name} - {self.image.name}"


class Room(models.Model):
    room_name = models.CharField(max_length=100, verbose_name="Название комнаты")
    room_hotel = models.ForeignKey(
        Hotel, on_delete=models.CASCADE, related_name="rooms", verbose_name="Отель"
    )
    ...
    room_type = models.CharField(
        max_length=16, choices=TYPE_ROOM, verbose_name="Тип комнаты"
    )
    ...
    room_status = models.CharField(
        max_length=15, choices=STATUS_CHOICES, verbose_name="Статус комнаты"
    )
    ...
    room_description = models.TextField(
        max_length=1000, verbose_name="Описание комнаты"
    )
    room_capacity = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(10)],
        verbose_name="Вместимость",
    )
    room_bed_count = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(10)],
...
class RoomImage(models.Model):
    ...
    room_image = models.ImageField(
        upload_to="room_images/", null=True, blank=True, verbose_name="Фото комнаты"
    )
...
class Review(models.Model):
    rev_user_name = models.ForeignKey(
        UserProfile, on_delete=models.CASCADE, verbose_name="Пользователь"
    )
    rev_hotel = models.ForeignKey(
        Hotel, on_delete=models.CASCADE, related_name="reviews", verbose_name="Отель"
    )
    ...
    rev_created_at = models.DateTimeField(
    ...
    rev_reply = models.TextField(
        blank=True, null=True, verbose_name="Ответ администрации"
    )
...
class Booking(models.Model):
    ...
    special_requests = models.TextField(
        blank=True, null=True, verbose_name="Пожелания и специальные запросы"
    )
